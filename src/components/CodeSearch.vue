<template>
  <div>
    <form v-if="termSearch">
      <input type="text" v-model="term" placeholder="Search by term" />
  ...
  methods: {
    async searchByTerms(e) {
      ...
      const res = await fetch(
        `https://clinicaltables.nlm.nih.gov/api/icd10cm/v3/search?sf=code,name&terms=${this.term}`
      );
    ...
    async searchByCode(e) {
      e.preventDefault();
      const res = await fetch(
        `http://www.icd10api.com/?code=${this.code}&desc=long&r=json`
      );
